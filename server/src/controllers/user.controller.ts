 ...
 */
export const createUserAccount: RequestHandler = catchAsync(async (req, res) => {
...
export const updateUserProfile = catchAsync(async (req, res) => {
  //let updateData:{name: string, email: string, bio: any, avatar: string }
  ...
  if (req.file) {
    const avatarResult = await uploadMedia(req.file.path);
     //updateData: updateData = { name, email: email?.toLowerCase(), bio, avatar: avatarResult?.secure_url || req.file.path };
    (updateData as updateData).avatar = avatarResult?.secure_url || req.file.path;
...
export const resetPassword: RequestHandler = catchAsync(async (req, res) => {
  ...
  const user = await User.findOne({
    resetPasswordToken: crypto.createHash("sha256").update(Buffer.from(token.toString())).digest("hex"),
