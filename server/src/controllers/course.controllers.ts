...
import { Lecture } from "../models/lecture.model.ts";
 ...
 */
export const createNewCourse: RequestHandler = catchAsync(async (req, res) => {
  const { title, subtitle, description, category, level, price }: { title: string, subtitle: string, description: string, category: string, level: string, price: number } = req.body;
...
export const getCourseLectures = catchAsync(async (req, res) => {
  ...
  const isEnrolled = course.enrolledStudents.includes(req.id as any);
  const isInstructor = course.instructor.toString() === req.id;

  let lectures = course.lectures.map as any;
  if (!isEnrolled && !isInstructor) {
    // Only return preview lectures for non-enrolled users
    lectures = lectures.filter((lecture: any) => lecture.isPreview);
  }
